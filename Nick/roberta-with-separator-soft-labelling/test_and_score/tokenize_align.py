...
def tokenize_n_align(data, tokenizer):
    ...
    for i in range(len(tokenized_inputs["input_ids"])):
        ...
        # Compute position shift for hard labels from "model_input + <@@>"
        length_of_question = len(data["model_input"][i]) + len(" <@@> ")
        ...
        masked_label_sequence = [0 if m == 1 else l for l, m in zip(label_sequence, attention_mask)] ## this set 0 for start symbol and separator <@@>, padding get -100
        ...
        if hard_labels:  # check if hard_labels is empty
            for idx, start_end in enumerate(offset_mapping):
                ...
                for (label_start, label_end) in hard_labels:
                    ## Nick
                    shifted_label_start = label_start + length_of_question
                    shifted_label_end = label_end + length_of_question

                    ## Nick
                    #if start >= label_start and end <= label_end:
                    if start >= shifted_label_start and end <= shifted_label_end:
